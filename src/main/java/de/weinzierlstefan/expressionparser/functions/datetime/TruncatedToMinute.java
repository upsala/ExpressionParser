...
public class TruncatedToMinute implements Function {
  ...
  public Value execute(ValueList valueList, ExecutorContext executorContext) throws ExpressionException {
    ...
    if (t instanceof LocalDateTime localDateTime) {
      return ValueTemporal.of(localDateTime.truncatedTo(ChronoUnit.MINUTES));
    }

    if (t instanceof LocalTime localTime) {
      return ValueTemporal.of(localTime.truncatedTo(ChronoUnit.MINUTES));
    }

    if (t instanceof OffsetDateTime offsetDateTime) {
      return ValueTemporal.of(offsetDateTime.truncatedTo(ChronoUnit.MINUTES));
    }

    if (t instanceof OffsetTime offsetTime) {
      return ValueTemporal.of(offsetTime.truncatedTo(ChronoUnit.MINUTES));
    }

    if (t instanceof ZonedDateTime zonedDateTime) {
      return ValueTemporal.of(zonedDateTime.truncatedTo(ChronoUnit.MINUTES));
    }

    if (t instanceof Instant instant) {
      return ValueTemporal.of(instant.truncatedTo(ChronoUnit.MINUTES));
    }
