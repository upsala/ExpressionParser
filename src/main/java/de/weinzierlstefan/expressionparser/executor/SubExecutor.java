...
public class SubExecutor implements Executor {
  ...
  public SubExecutor(Executor left, Executor right) {
    this.left = left;
    this.right = right;
  }
  ...
  public Value exec(ExecutorContext ctx) throws ExpressionException {
    ...
    if (leftValue.isNumber() && rightValue.isNumber()) {
      ...
      if (leftValue.isLong() || rightValue.isLong()) {
        return ValueLong.of(Math.subtractExact(leftValue.getLong(), rightValue.getLong()));
      }

      if (leftValue.isInt() || rightValue.isInt()) {
        return ValueInt.of(Math.subtractExact(leftValue.getInt(), rightValue.getInt()));
      }
    }

    throw new ExpressionException(
      String.format(
        "Subtraction of '%s' and '%s' is not possible",
        leftValue.getType(),
        rightValue.getType()
      )
    );
